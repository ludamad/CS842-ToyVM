...
local function table2ffi(table, type)
    local ctype = ffi.new(type, #table)
    append(remembered, ctype)
    for i=1,#table do
        ctype[i - 1] = table[i] -- Note, C arrays are off by one from Lua!!
    end
    return ctype
end 
...
function M.Function:call(func, name, args) 
    local cparams = table2ffi(args, "jit_value_t[?]")
    if getmetatable(func) == M.NativeFunction then
        return libjit.jit_insn_call_native(self.func, name, func.func, func.signature, cparams, #args, 0)
    else
        return libjit.jit_insn_call(self.func, name, func.func, func.signature, cparams, #args, 0)
    end
end
...
function M.Function:_not(value) 
    return libjit.jit_insn_not(self.func, value)
end

function M.Function:_or(value1, value2) 
    return libjit.jit_insn_or(self.func, value1, value2)
end

M.Context = newtype()
function M.Context:init()
    self.context = libjit.jit_context_create()
end

local function drop(str, ...)
...
local function toCamel(str)
    ...
    for i=2,#parts do
        parts[i] = parts[i]:gsub("^(.)", string.upper)
    end
    return table.concat(parts) 
end

    -- Make really sure LuaJIT treats these as fixed C functions via code generation
local methodSources = {}

local function addMethod(parts)
    table.insert(methodSources, table.concat(parts)) 
end

local function compileMethods()
...
ffi.cdef [[
   ...
   ;
    ...
    extern jit_opcode_info_t const jit_opcodes[439];
